...
| `imageFilter` | yes | `with_image_filter`: a blur, a matrix, dilate, erode, a color filter and any composition of them, applied to what the paint drew rather than to the color it computed. Not a runtime effect, which upstream's `DlImageFilter` also offers; see `docs/non-parity.md` §7 | `composing_an_erosion_with_a_dilation_depends_on_which_runs_first`, `a_color_filter_can_be_half_of_an_image_filter_composition` |
